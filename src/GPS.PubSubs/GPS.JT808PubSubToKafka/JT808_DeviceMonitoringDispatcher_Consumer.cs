...
{
    public class JT808_DeviceMonitoringDispatcher_Consumer : JT808MsgIdConsumerBase
    {
        ...
        public JT808_DeviceMonitoringDispatcher_Consumer(ILoggerFactory loggerFactory) : base(loggerFactory)
        {
            Logger = loggerFactory.CreateLogger<JT808_DeviceMonitoringDispatcher_Consumer>();
            consumer = new Consumer<string, byte[]>(Config, new StringDeserializer(Encoding.UTF8), new ByteArrayDeserializer());
            RegisterEvent();
        }

        public JT808_DeviceMonitoringDispatcher_Consumer(Dictionary<string, object> config, ILoggerFactory loggerFactory) : base(config, loggerFactory)
        {
            Logger = loggerFactory.CreateLogger<JT808_DeviceMonitoringDispatcher_Consumer>();
            consumer = new Consumer<string, byte[]>(Config, new StringDeserializer(Encoding.UTF8), new ByteArrayDeserializer());
            RegisterEvent();
