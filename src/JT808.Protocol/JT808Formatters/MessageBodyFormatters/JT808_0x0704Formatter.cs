...
namespace JT808.Protocol.JT808Formatters.MessageBodyFormatters
{
    public class JT808_0x0704Formatter : IMessagePackFormatter<JT808_0x0704>
    {
        public JT808_0x0704 Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize)
        {
            ...
            jT808_0X0704.Count = BinaryExtensions.ReadUInt16Little(bytes, offset);
            offset = offset + 2;
            jT808_0X0704.LocationType= (JT808_0x0704.BatchLocationType)BinaryExtensions.ReadByteLittle(bytes, offset);
            offset = offset + 1;
            List<JT808_0x0200> jT808_0X0200s = new List<JT808_0x0200>();
            int bufReadSize;
            for (int i = 0; i < jT808_0X0704.Count; i++)
            {
                int buflen = BinaryExtensions.ReadUInt16Little(bytes, offset);
                offset = offset + 2;
                try
                {
        ...
        {
            offset += BinaryExtensions.WriteLittle(ref bytes, offset, value.Count);
            offset += BinaryExtensions.WriteLittle(ref bytes, offset, (byte)value.LocationType);
            foreach (var item in value?.Positions)
            {
                try
                {
                    ...
                    BinaryExtensions.WriteLittle(ref bytes, offset, (ushort)(positionOffset - offset - 2));
