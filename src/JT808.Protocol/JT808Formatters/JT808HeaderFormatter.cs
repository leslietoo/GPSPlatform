...
namespace JT808.Protocol.JT808Formatters
{
    ...
    public class JT808HeaderFormatter : IMessagePackFormatter<JT808Header>
    {
        public JT808Header Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize)
        {
            ...
            // 1.消息ID
            jT808Header.MsgId =(JT808MsgId)BinaryExtensions.ReadUInt16Little(bytes, offset);
            offset = offset + 2;
            // 2.消息体属性
            ...
            offset = offset + 2;
            // 3.终端手机号 (写死大陆手机号码)
            jT808Header.TerminalPhoneNo = BinaryExtensions.ReadBCD(bytes, offset, 6).ToString().PadLeft(12, '0');
            offset = offset + 6;
            // 4.消息流水号
            jT808Header.MsgNum = BinaryExtensions.ReadUInt16Little(bytes, offset);
            offset = offset + 2;
            readSize = offset;
            return jT808Header;
        }
